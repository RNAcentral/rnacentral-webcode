...
from portal.models import EnsemblAssembly, SequenceRegion
...
def update_example_locations():
    ...
    for assembly in EnsemblAssembly.objects.filter().all():
        ...
        if assembly.ensembl_url in example_locations:
            assembly.example_chromosome = example_locations[assembly.ensembl_url][
                "chromosome"
            ]
        ...
        try:
            region = (
                SequenceRegion.objects.filter(assembly_id=assembly.assembly_id)
                .all()[:1]
                .get()
            )
