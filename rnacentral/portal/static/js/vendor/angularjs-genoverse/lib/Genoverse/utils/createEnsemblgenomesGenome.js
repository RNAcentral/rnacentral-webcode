...
require('http').get({
  hostname : 'rest.ensemblgenomes.org',
  path     : '/info/assembly/bombyx_mori?bands=1',
  ...
  response.on('end', function () {
    try {
      require('fs').writeFile('js/genomes/bombyx_mori.js', JSON.stringify(JSON.parse(str).top_level_region.filter(function (d) {
        return d.coord_system === 'supercontig';
      }).map(function (d) {
        return [
          d.name, {
            size: d.length,
