...
angular.module("rnacentralApp").service('genoverse', ['$filter', function($filter) {
    return {
        Genoverse: Genoverse,
        /**
         * The following functions assume availability of $scope variable in outer scope.
         */
        urls: {
        ...
        RNAcentralParseData: function(data) {
        ...
        genesPopulateMenu: function(feature) {
        ...
        transcriptsPopulateMenu: function(feature) {
            var chrStartEnd = feature.seq_region_name + ':' + feature.start + '-' + feature.end; // string e.g. 'X:1-100000'
            var location = '<a href="http://' + $scope.domain + '/' + $filter('urlencodeSpecies')($scope.genome.species) + '/Location/View?r=' + chrStartEnd + '" id="ensembl-link" target="_blank">' + chrStartEnd + '</a>';
        ...
        RNAcentralPopulateMenu: function(feature) {
            var chrStartEnd = feature.seq_region_name + ':' + feature.start + '-' + feature.end; // string e.g. 'X:1-100000'
            var location = '<a href="http://' + $scope.domain + '/' + $filter('urlencodeSpecies')($scope.genome.species) + '/Location/View?r=' + chrStartEnd + '" id="ensembl-link" target="_blank">' + chrStartEnd + '</a>';
         ...
         */
        getEnsemblEndpoint: function(species) {
            var ensemblSpecies = [
         ...
         */
        getEnsemblSubdomainByDivision: function(genome) {
            var subdomain;
