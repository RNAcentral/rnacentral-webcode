...
class RnaEndpointsTestCase(ApiV1BaseClass):
    ...
    * /rna/id/2d/svg
    ...
    def test_related_proteins_pagination(self):
        ...
        self.assertGreater(
            response.data["count"], 8000
        )  # well, not quite 'over 9000', but still many...
        self.assertEqual(response.status_code, 200)
        self.assertTrue(
    ...
    def _test_rna_svg_image_404(self):
        ...
        response = self.client.get(
            reverse("rna-2d-svg", kwargs={"pk": "URS0000000002"})
        )
    ...
    def test_rna_xrefs_species_specific(self):
        ...
        url = reverse(
            ...
            kwargs={"pk": "URS00006457C1", "taxid": "10090"},
        )
        self._test_url(url)
    ...
    def _test_rna_2d_species_specific(self):
        ...
        url = reverse(
            "rna-2d-species-specific", kwargs={"pk": "URS00006457C1", "taxid": "10090"}
        )
        self._test_url(url)
    ...
    def test_rna_genome_locations(self):
        ...
        url = reverse(
            "rna-genome-locations", kwargs={"pk": "URS00006457C1", "taxid": "10090"}
        )
        self._test_url(url)
    ...
    def test_rna_go_annotations(self):
        ...
        url = reverse(
            "rna-go-annotations", kwargs={"pk": "URS00006457C1", "taxid": "10090"}
        )
        self._test_url(url)
    ...
    def test_ensembl_compara(self):
        ...
        url = reverse(
            "rna-ensembl-compara", kwargs={"pk": "URS00006457C1", "taxid": "10090"}
        )
        self._test_url(url)
    ...
    def test_rna_lncrna_targets(self):
        ...
        url = reverse(
            "rna-lncrna-targets", kwargs={"pk": "URS00006457C1", "taxid": "10090"}
        )
        self._test_url(url)
...
class DatabaseSpecificXrefsTestCase(ApiV1BaseClass):
    ...
    def _test_mirbase_mature_products(self):
        self._test_time_and_existence(
            "URS0000759B7E", self.timeout, "mirbase_mature_products"
        )

    # TODO: Review this test, because sometimes it fails
    def _test_mirbase_precursor(self):
        self._test_time_and_existence(
            "URS00006457C1", self.timeout, "mirbase_precursor"
        )
...
class OutputFormatsTestCase(ApiV1BaseClass):
    ...
    def test_genome_annotations(self):
        ...
        response = self._test_url(
            "/api/v1/overlap/region/homo_sapiens/2:39,745,816-39,826,679"
        )
...
class SpeciesSpecificIdsTestCase(ApiV1BaseClass):
    ...
    def test_species_specific_id(self):
        ...
        url = reverse(
            "rna-species-specific", kwargs={"pk": self.upi, "taxid": str(self.taxid)}
        )
        c = APIClient()
        response = c.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.data["rnacentral_id"], "%s_%i" % (self.upi, self.taxid)
        )
    ...
    def test_nonexistent_taxid(self):
        ...
        taxid = 00000
        url = reverse(
            "rna-species-specific", kwargs={"pk": self.upi, "taxid": str(taxid)}
        )
        c = APIClient()
        response = c.get(url)
        self.assertEqual(response.status_code, 404)
